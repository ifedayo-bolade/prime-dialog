...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val list = ArrayList<String>(listOf(*strings))
        val adapter = SampleAdapter(this, list)
    ...
    fun demoListenerDialog() {
        PrimeDialog(this)
            ...
            .setOnDialogDismissListener { _: PrimeDialog, actionId: Int, isCancelled: Boolean ->
                val string = when (actionId) {
                    PrimeDialog.DISMISS_ACTION_INTERNAL -> "INTERNAL"
                    PrimeDialog.DISMISS_ACTION_BACK_PRESSED -> "BACK_PRESSED"
                    PrimeDialog.DISMISS_ACTION_CLICK_OUTSIDE -> "OUTSIDE_CLICK"
                    PrimeDialog.DISMISS_ACTION_POSITIVE_BUTTON -> "POSITIVE_BUTTON"
                    PrimeDialog.DISMISS_ACTION_NEGATIVE_BUTTON -> "NEGATIVE_BUTTON"
                    PrimeDialog.DISMISS_ACTION_NEUTRAL_BUTTON -> "NEUTRAL_BUTTON"
                    else -> "UNKNOWN"
                }
    ...
    fun demoCustomDialog() {
        PrimeDialog(this)
            .setCustomView(R.layout.custom_dialog_layout)
            .setOnDialogShowListener { dialog: PrimeDialog ->
                showLoginGuide()
                val loginButton = dialog.findViewById<Button>(R.id.btnLogin)
                loginButton.setOnClickListener { _: View ->
                    loginButton.text = "HEY, I'M A DUMMY BUTTON"
    ...
    private fun showLoginGuide() {
        showMessage("Please click the login button")
    }

    private val onDontShowAgainListener: OnDontShowAgainListener =
        object : OnDontShowAgainListener() {
            override fun onBoxCheck(isChecked: Boolean) {
                showMessage(if (isChecked) "Checked" else "Not checked")
            }
